# Usage: python3 bad-contracts-per-country-2.py [Country]
...
#print(BASE_URL, TENANT_ID, USERNAME, PASSWORD)
#print()
...
print(f"Report on units with missing or incorrect metadata per country\n")
num_bad_contracts = 0
bad_ups = {}
countries = {}
...
for country in bad_ups_sorted:
  ...
  for company in bad_ups_sorted[country]:
    ...
    for ser_num in bad_ups_sorted[country][company]:
      ...
      num_bad_contracts += 1
  bad_percent = num_bad_contracts / ups_count[country]
  if bad_percent > THRESHOLD:
    flag = "*** BAD ***"
  else:
    flag = "<<< GOOD <<<"
  print(f"Number of bad UPSs per country: {num_bad_contracts} out of {ups_count[country]} or {num_bad_contracts/ups_count[country]:.1%} {flag}")
  print("  ")
